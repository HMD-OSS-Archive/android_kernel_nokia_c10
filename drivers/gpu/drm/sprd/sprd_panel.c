...
/*Modified by Qinjinke for touchscreen double tap 2020-11-19 begin*/
#include <linux/proc_fs.h>  
#include <asm/uaccess.h>
/*Modified by Qinjinke for touchscreen double tap 2020-11-19 end*/
/*[COMMON COMMIT] ODM add for hmd lcd hwinfo config begin QJK*/
#ifdef CONFIG_CUSTOM_HWINFO
#include <hwinfo/hwinfo.h>
#endif
/*[COMMON COMMIT] ODM add for hmd lcd hwinfo config end QJK*/
...
/*Modified by Qinjinke for touchscreen double tap 2020-11-19 begin*/
static uint32_t fts_double_click = 0;
static struct proc_dir_entry *fts_double_click_proc = NULL;
static ssize_t fts_double_click_proc_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char buf[50]={0};
	int count = 0;
	...
	count = sprintf(buf, "fts_double_click is %d\n", fts_double_click);
	if (copy_to_user(page, buf, count))
	{
		return -EFAULT;
	}
	*ppos +=count;
...
static ssize_t fts_double_click_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
	char inbuffer[4]={0};
	if(count>4) {
	    count=4;
	}
	copy_from_user(inbuffer, buf, count);
	if (sscanf(inbuffer, "%d",  &fts_double_click) == 1) 
	{
		printk("**fts_double= %d **\n",fts_double_click);
	}
	if(fts_double_click==1)//A
	{
		printk("**0000000000000000000000000= %d **\n",fts_double_click);
	}else{
		printk("**1111111111111111111111111= %d **\n",fts_double_click);
	}
	return count;
...
static const struct file_operations config_proc_ops = {
    .owner = THIS_MODULE,
    .read = fts_double_click_proc_read,
    .write = fts_double_click_proc_write,
};
/*Modified by Qinjinke for touchscreen double tap 2020-11-19 end*/
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	DRM_INFO("%s()\n", __func__);
    if (!fts_double_click){/*Modified by Qinjinke for touchscreen double tap 2020-11-19*/
	...
	struct sprd_panel *panel)
{
	...
	/*[COMMON COMMIT] ODM add for hmd lcd hwinfo config begin QJK*/
	#ifdef CONFIG_CUSTOM_HWINFO   
	rc = of_property_read_string(lcd_node, "sprd,lcm-ic-name", &str);
	if (!rc)
		info->ic_name= str;
	rc = of_property_read_string(lcd_node, "sprd,lcm-ic-vendor", &str);
	if (!rc)
		info->vendor_name = str;
	rc = of_property_read_string(lcd_node, "sprd,lcm-ic-inch", &str);
	if (!rc)
		info->ic_inch = str;
	pr_info("ic_name=%s, vendor_name=%s, ic_inch=%s\n", info->ic_name, info->vendor_name, info->ic_inch);

	custom_hwinfo_add("lcm_ic", (char*)info->ic_name);
	custom_hwinfo_add("lcm_vendor",(char*)info->vendor_name);
	custom_hwinfo_add("lcm_inch",(char*)info->ic_inch);
	#endif
	/*[COMMON COMMIT] ODM add for hmd lcd hwinfo config end QJK*/
...
static int sprd_panel_probe(struct mipi_dsi_device *slave)
{
	...
	panel->is_enabled = true;
    /*Modified by Qinjinke for touchscreen double tap 2020-11-19 begin*/
    fts_double_click_proc = proc_create("fts_double_click", 0666, NULL, &config_proc_ops);
	if (fts_double_click_proc == NULL)
	{
		printk("create_proc_entry %s failed\n", "fts_double_click");
	}
	else
	{
		printk("create proc entry %s success", "fts_double_click");
	}
    /*Modified by Qinjinke for touchscreen double tap 2020-11-19 end*/
