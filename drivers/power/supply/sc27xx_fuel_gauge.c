...
static int bset_temp_value=0xffff;
...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
    ...
    if (!is_first_poweron && *cap != SC27XX_FGU_RTC2_RESET_VALUE) {
		data->boot_cap = *cap;
	...
	*cap = delta_cap + data->init_cap;
	...
	if (data->cap_table_len > 0) {
		...
		*cap = DIV_ROUND_CLOSEST((*cap + temp_cap - 1000) * 1000, temp_cap);
		/*test 55913 [MNT-347] charge cap modify persent 0 begin QJK*/
		if(*cap<0)
		    *cap=0;
		/*test 55913 [MNT-347] charge cap modify persent 0 end QJK*/
	}
...
static int sc27xx_fgu_get_temp(struct sc27xx_fgu_data *data, int *temp)
{
    ...
    /*drv nobug add modify for tempture use adb set test begin QJK*/
    if (bset_temp_value>=-400 && bset_temp_value<=800) {
        *temp = bset_temp_value;
    }
    /*drv nobug add modify for tempture use adb set test end QJK*/
	data->bat_temp = *temp;
    dev_info(data->dev, "sc27xx_fgu_get_temp  bat_temp = %d, bset_temp_value = %d, temp = %d\n",data->bat_temp,bset_temp_value,*temp);
	return 0;
}
/*drv nobug add modify for tempture use adb set test begin QJK*/
/*dir: /sys/devices/platform/charger-manager/power_supply/battery/temp*/
static int sc27xx_fgu_set_temp(struct sc27xx_fgu_data *data, int temp)
{
       bset_temp_value=temp;
       dev_info(data->dev, "sc27xx_fgu_set_temp  value = %d\n",bset_temp_value);
       return 0;
}
/*drv nobug add modify for tempture use adb set test end QJK*/
				   ...
				   const union power_supply_propval *val)
{
	...
	/*drv nobug add modify for tempture use adb set test begin QJK*/
    case POWER_SUPPLY_PROP_TEMP:
        dev_info(data->dev, "fgu POWER_SUPPLY_PROP_TEMP value %d\n",val->intval);
        sc27xx_fgu_set_temp(data,val->intval);
        ret=0;  
        break;
    /*drv nobug add modify for tempture use adb set test end QJK*/
	default:
