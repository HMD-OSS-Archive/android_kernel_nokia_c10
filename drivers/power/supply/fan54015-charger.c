...
/*Added by FAE QinJinke 20201112 begin*/
...
fan54015_charger_set_termina_vol(struct fan54015_charger_info *info, u32 vol)
{
	...
	if (vol <= 4050)/*BBtest 63055 ODM add for warm charge voltage max QJK*/
		reg_val = 0x0;
	else if (vol >= 4440)
		reg_val = 0x2e;
	else
...
static int fan54015_charger_hw_init(struct fan54015_charger_info *info)
{
	...
	if (ret) {
		...
		ret = fan54015_update_bits(info, FAN54015_REG_5,
					   ...
					   0x2);  // vsp is set to  4.366V /*Added by FAE QinJinke 20201112*/
		if (ret) {
					...
					u32 cur)
{
	...
	if (cur < 550000)/*Added by FAE QinJinke 20201112*/
		reg_val = 0x0;
	else if (cur >= 550000 && cur < 750000)/*Added by FAE QinJinke 20201112*/
		reg_val = 0x1;
	else if (cur >= 750000 && cur < 850000)
		reg_val = 0x2;
	else if (cur >= 850000 && cur < 1050000)
					...
					u32 *cur)
{
    ...
    fan54015_dump_registers(info);/*Added by FAE QinJinke 20201112*/
	...
	switch (reg_val) {
	case 0:
		*cur = 500000;/*Added by FAE QinJinke 20201112*/
		break;
	case 1:
		*cur = 550000;/*Added by FAE QinJinke 20201112*/
		break;
	case 2:
		*cur = 750000;
		break;
				   ...
				   u32 limit_cur)
{
	...
	if (limit_cur <= 100000)
		...
		reg_val = 0x3;
	/*Added by FAE QinJinke 20201112 begin*/
	//Solve usb type error problem
	if (reg_val == 0x0)
		reg_val = 0x3; 
	//  Solve usb type error problem
	/*Added by FAE QinJinke 20201112 end*/	
	ret = fan54015_update_bits(info, FAN54015_REG_1,
...
static int fan54015_charger_enable_otg(struct regulator_dev *dev)
{
    ...
    ret = fan54015_charger_start_charge(info); /*Added by FAE QinJinke 20201112*/
	ret = fan54015_update_bits(info, FAN54015_REG_1,
				   FAN54015_REG_HZ_MODE_MASK |
				   FAN54015_REG_OPA_MODE_MASK,
				   FAN54015_REG_OPA_MODE_MASK);
