...
/*[COMMON]ODM add for ilitek touchpanel charge noise config begin QJK*/
#ifdef CONFIG_TOUCHPANEL_CHARGE_NOISE
#include <hwinfo/cus_notifier.h>
#endif
/*[COMMON]ODM add for ilitek touchpanel charge noise config end QJK*/
/*
...
static int cm_check_charge_voltage(struct charger_manager *cm)
{
	...
	union power_supply_propval val;
	int ret, charge_vol;
/*Added by QinJinke for fan54015 ovp 20201112 begin*/	
#ifdef CONFIG_CHARGER_FAN54015
	int charge_cur;
#endif	
/*Added by QinJinke for fan54015 ovp 20201112 end*/
			...
			enum cm_event_types type)
{
	...
	if (is_ext_pwr_online(cm)) {
		...
		/*[COMMON]ODM add for ilitek touchpanel charge noise config begin QJK*/
		#ifdef CONFIG_TOUCHPANEL_CHARGE_NOISE
        cable_notifier_call_chain(1,NULL);
        #endif
		/*[COMMON]ODM add for ilitek touchpanel charge noise config end QJK*/
		...
		/*[COMMON]ODM add for ilitek touchpanel charge noise config begin QJK*/
		#ifdef CONFIG_TOUCHPANEL_CHARGE_NOISE
		cable_notifier_call_chain(0,NULL);
		#endif
		/*[COMMON]ODM add for ilitek touchpanel charge noise config end QJK*/
		...
		union power_supply_propval *val)
{
	...
	int ret = 0;
	int i;
/*added by huangwenjian for system can boot normally at 0% battery when charging 20210315 begin*/ 	
#ifdef CONFIG_CHARGER_BQ2560X	
	int batt_uV = 0;
#endif	
/*added by huangwenjian for system can boot normally at 0% battery when charging 20210315 end*/ 
...
/*added by huangwenjian for system can boot normally at 0% battery when charging 20210315 begin*/ 			
#ifdef CONFIG_CHARGER_BQ2560X				
        else if (is_charging(cm) && batt_uV > 3400000)
            val->intval = POWER_SUPPLY_CAPACITY_LEVEL_NORMAL;	
#endif      
/*added by huangwenjian for system can boot normally at 0% battery when charging 20210315 end*/  	
		else
		     ...
		     const union power_supply_propval *val)
{
	...
	/*drv nobug add modify for tempture use adb set test begin QJK*/
	struct power_supply *fuel_gauge = NULL;
	/*drv nobug add modify for tempture use adb set test end QJK*/
	int cur_jeita_status;
	int ret = 0;
	int i;
    ...
    /*drv nobug add modify for tempture use adb set test begin QJK*/
    /*dir: /sys/devices/platform/charger-manager/power_supply/battery/temp*/
    case POWER_SUPPLY_PROP_TEMP:
        fuel_gauge = power_supply_get_by_name(
        cm->desc->psy_fuel_gauge);
        if (!fuel_gauge) {
            dev_err(cm->dev, "Cannot find power supply temp \"%s\"\n",
            cm->desc->psy_fuel_gauge);
            ret = -ENODEV;
            break;
        }
        ...
        if (!ret) {
            power_supply_put(fuel_gauge);
            dev_info(cm->dev, "success to set POWER_SUPPLY_PROP_TEMP value %d\n",val->intval);
            break;
        }
        dev_err(cm->dev,"failed set POWER_SUPPLY_PROP_TEMP\n ");
        break;
    /*drv nobug add modify for tempture use adb set test end QJK*/
	default:
		ret = -EINVAL;
