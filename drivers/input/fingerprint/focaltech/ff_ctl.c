...
ff_driver_config_t *g_config_t = NULL;
...
int ff_log_printf(ff_log_level_t level, const char *tag, const char *fmt, ...)
{
    ...
    /* Send to ff_device. */
    if (likely(g_context) && likely(g_config_t) && unlikely(g_config_t->logcat_driver)) {
...
#ifdef CONFIG_PM_WAKELOCKS
    __pm_wakeup_event(&g_context->wake_lock, jiffies_to_msecs(5*HZ));
#else
    wake_lock_timeout(&g_context->wake_lock, 5 * HZ); // 2 seconds.
#endif
...
static irqreturn_t ff_ctl_device_irq(int irq, void *dev_id)
{
    ...
    if (likely(irq == ctx->irq_num)) {
        if (g_config_t && g_config_t->enable_fasync && g_context->async_queue) {
...
static int ff_ctl_register_input(void)
{
    ...
    if (g_config_t) {
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_nav_left    );
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_nav_right   );
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_nav_up      );
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_nav_down    );
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_double_click);
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_click       );
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_long_press  );
        input_set_capability(g_context->input, EV_KEY, g_config_t->keycode_simulation  );
    }
...
static int ff_ctl_init_driver(void)
{
    ...
    do {
        ...
        if (g_config_t && g_config_t->enable_spidev) {
#ifdef FF_SPI_SET
            err = ff_spi_init();
            if (err) {
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_SYNC_CONFIG: {
        ...
        g_config_t = &driver_config;

        /* Take logging level effect. */
        g_log_level = 1;//g_config_t->log_level;
        break;
    }
