...
#include <linux/pinctrl/consumer.h>/*Qinjinke Add Modify for TP Gesture 2020-11-16*/
...
/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
void fts_irq_eic_disable(void)
{
    unsigned long irqflags;

    FTS_FUNC_ENTER();
    spin_lock_irqsave(&fts_data->irq_eic_lock, irqflags);

    if (!fts_data->irq_eic_disabled) {
	irq_set_status_flags(fts_data->irq_eic, IRQ_DISABLE_UNLAZY);
        disable_irq_nosync(fts_data->irq_eic);
        fts_data->irq_eic_disabled = true;
    }

    spin_unlock_irqrestore(&fts_data->irq_eic_lock, irqflags);
    FTS_FUNC_EXIT();
}

void fts_irq_eic_enable(void)
{
    unsigned long irqflags = 0;
    ...
    if (fts_data->irq_eic_disabled) {
        enable_irq(fts_data->irq_eic);
        fts_data->irq_eic_disabled = false;
    }

    spin_unlock_irqrestore(&fts_data->irq_eic_lock, irqflags);
    FTS_FUNC_EXIT();
}
/*Qinjinke Add Modify for TP Gesture 2020-11-16 end*/
...
/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
static irqreturn_t fts_irq_eic_handler(int irq, void *data)
{
//struct fts_ts_data *ts_data = fts_data;
#if defined(CONFIG_PM) && FTS_PATCH_COMERR_PM
    int ret = 0;
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
	...
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
    ts_data->irq_eic = gpio_to_irq(pdata->irq_use_eic);
    //gpio_set_debounce(pdata->irq_use_eic,0);//use eic_debounce pin should set,use eic_async pin do not set
    pdata->irq_use_eic_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND;
    printk("zqihui:: irq_eic:%d, eic_flag:%x", ts_data->irq_eic, pdata->irq_use_eic_flags);
    ret = request_threaded_irq(ts_data->irq_eic, NULL, fts_irq_eic_handler,pdata->irq_use_eic_flags,FTS_DRIVER_NAME, ts_data);
    fts_irq_eic_disable();
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 end*/
    return ret;
}
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
	...
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
     if (gpio_is_valid(data->pdata->irq_use_eic)) {
		printk("zqihui::fts_gpio_configure3\n");
        ret = gpio_request(data->pdata->irq_use_eic, "fts_irq_eic");
        if (ret) {
            FTS_ERROR("[GPIO]irq gpio request failed");
            goto err_irq_gpio_req;
        }
	printk("zqihui::fts_gpio_configure4\n");
        ret = gpio_direction_input(data->pdata->irq_use_eic);
        if (ret) {
            FTS_ERROR("[GPIO]set_direction for irq gpio failed");
            goto err_irq_gpio_dir;
        }
...
/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
static int fts_irq_pinctrl_init(struct fts_ts_data *ts)
{
    int ret = 0;
    FTS_FUNC_ENTER();
    ts->pinctrl = devm_pinctrl_get(ts->dev);
    ...
    ts->pins_extconf0 = pinctrl_lookup_state(ts->pinctrl, "tp_extconf0");
    if (IS_ERR_OR_NULL(ts->pins_extconf0)) {
        FTS_ERROR("Pin state[extconf0] not found");
        ret = PTR_ERR(ts->pins_extconf0);
        goto err_pinctrl_lookup;
    }

    ts->pins_extconf1 = pinctrl_lookup_state(ts->pinctrl, "tp_extconf1");
    if (IS_ERR_OR_NULL(ts->pins_extconf1)) {
        FTS_ERROR("Pin state[extconf1] not found");
        ret = PTR_ERR(ts->pins_extconf1);
        goto err_pinctrl_lookup;
    }
    FTS_FUNC_EXIT();
    return 0;
...
err_pinctrl_get:
    ts->pinctrl = NULL;
    ts->pins_extconf0 = NULL;
    ts->pins_extconf1 = NULL;
    FTS_FUNC_EXIT();
    return ret;
}

static int fts_pinctrl_select_irq_gpio(struct fts_ts_data *ts)
{
    int ret = 0;

    if (ts->pinctrl && ts->pins_extconf1) {
        ret = pinctrl_select_state(ts->pinctrl, ts->pins_extconf1);
	 printk("fts_pinctrl_select_irq_gpio %d\n",ret);
        if (ret < 0) {
            FTS_ERROR("Set irq gpio pin state error:%d", ret);
        }
    }
...
static int fts_pinctrl_select_irq_eic(struct fts_ts_data *ts)
{
    int ret = 0;

    if (ts->pinctrl && ts->pins_extconf0) {
        ret = pinctrl_select_state(ts->pinctrl, ts->pins_extconf0);
	 printk("fts_pinctrl_select_irq_eic %d\n",ret);
        if (ret < 0) {
            FTS_ERROR("Set irq eic pin state error:%d", ret);
        }
    }
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    ret = of_property_read_u32(np, "debounce-interval",&pdata->debounce_interval);
	if (ret < 0)
		pdata->debounce_interval = 1;
    pdata->irq_gpio = of_get_named_gpio_flags(np, "focaltech,gpio-gpio",0, &pdata->irq_gpio_flags);
    if (pdata->irq_gpio < 0)
        FTS_ERROR("Unable to get irq_gpio");

    pdata->irq_use_eic = of_get_named_gpio_flags(np, "focaltech,irq-gpio", 0, &pdata->irq_use_eic_flags);
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
	ret = fts_irq_pinctrl_init(ts_data);
	if (ret) {
	    FTS_ERROR("fts_irq_pinctrl_init fail");
	}
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 end*/
#if FTS_POWER_SOURCE_CUST_EN
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    free_irq(ts_data->irq_eic, ts_data);/*Qinjinke Add Modify for TP Gesture 2020-11-16*/
    input_unregister_device(ts_data->input_dev);
...
int fts_ts_suspend(struct device *dev)
{
	...
	fts_irq_disable();
	fts_irq_eic_enable();
	fts_pinctrl_select_irq_eic(ts_data);
...
/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/
extern int sleep_set_gesture_flag;
extern int sleep_set_gesture_value;
static void focaltech_status_in_sleep_set(void){
    if(1 == sleep_set_gesture_flag){
        FTS_INFO("sleep_set_gesture_value : [%d]\n", sleep_set_gesture_value);
        fts_data->gesture_mode = sleep_set_gesture_value;
        FTS_INFO("fts_data->gesture_mode = [%d]\n",  fts_data->gesture_mode);
    }
    sleep_set_gesture_flag = 0;
    sleep_set_gesture_value = fts_data->gesture_mode;
}
/*Qinjinke Add Modify for TP Gesture 2020-11-16 end*/
...
int fts_ts_resume(struct device *dev)
{
	...
	fts_irq_eic_disable();
	fts_irq_enable();
	fts_pinctrl_select_irq_gpio(ts_data);
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 end*/
