...
/*Modified by Qinjinke for touchscreen suspend/resume 2020-11-17 begin*/
...
/*Modified by Qinjinke for touchscreen suspend/resume 2020-11-17 end*/
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
    int ret = 0;
    
    /*Modified by Qinjinke for touchscreen suspend/resume 2020-11-17 begin*/
    static struct class *sprd_touchscreen_class;
    struct device *touchscreen_dev;
    sprd_touchscreen_class = class_create(THIS_MODULE, "touchscreen");
    if (IS_ERR(sprd_touchscreen_class)) {
        pr_warn("Unable to create sprd_touchscreen_class; errno = %ld\n",
        PTR_ERR(sprd_touchscreen_class));
        return PTR_ERR(sprd_touchscreen_class);
    }
    touchscreen_dev = device_create(sprd_touchscreen_class, NULL, 0, NULL, "light_suspend");
    if (IS_ERR(touchscreen_dev))
        pr_err("Failed to create device(touchscreen)!\n");
    if (device_create_file(touchscreen_dev, &dev_attr_ts_suspend) < 0)        
        pr_err("Failed to create device file(%s)!\n", dev_attr_ts_suspend.attr.name);
    /*Modified by Qinjinke for touchscreen suspend/resume 2020-11-17 end*/
