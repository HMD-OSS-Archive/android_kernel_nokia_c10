...
static int proc_fts_gesture_show(struct file *file, char __user *pBuffer, size_t nCount, loff_t *pPos);
static int proc_fts_gesture_store(struct file *file, const char *pBuffer, size_t nCount, loff_t *pPos);
...
/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/ 
static struct file_operations fts_gesture_mode_proc_fops = {
       .write = proc_fts_gesture_store,
       .read = proc_fts_gesture_show,
};

/************************************************************************
* Name: proc_fts_gesture_show
*  Brief:
...
static int proc_fts_gesture_show(struct file *file, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    if(copy_to_user(pBuffer, buf, count))
	{
        return -EFAULT;
	}	
...
* Return:
***********************************************************************/
int sleep_set_gesture_flag = 0;
int sleep_set_gesture_value = 0;
...
static int proc_fts_gesture_store(struct file *file,const char *pBuffer, size_t nCount, loff_t *pPos) 
{
    ...
    int mode_state = 0;  
    struct fts_ts_data *ts_data = fts_data;
    memset(temp_file, 0, 64);               
    if (copy_from_user(temp_file, pBuffer,  sizeof(temp_file)))
    {
        return -EFAULT;
    }
  
    if(sscanf(temp_file, "%d",&mode_state))
    {
        mutex_lock(&ts_data->input_dev->mutex);
        if(true == ts_data->suspended){
            if(1 == mode_state || 0 == mode_state){
                FTS_INFO("[GESTURE] set in sleep ,save it mode_state : [%d]\n",mode_state);
                sleep_set_gesture_value = mode_state;
                sleep_set_gesture_flag = 1;
            }
        }
        else{
            if (mode_state == 1) {
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_DOUBLECLICK:
        gesture = KEY_F22;/*Qinjinke Add Modify for TP Gesture 2020-11-16*/
        break;
    case GESTURE_O:
...
int fts_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    fts_reset_proc(200);//jinke add for double-tap 2020-11-20
    FTS_FUNC_EXIT();
    return 0;
}
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    input_set_capability(input_dev, EV_KEY, KEY_F22);/*Qinjinke Add Modify for TP Gesture 2020-11-16*/
	...
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 begin*/  
	proc_create("driver/fts_gesture_mode", 0666, NULL, &fts_gesture_mode_proc_fops);
	/*Qinjinke Add Modify for TP Gesture 2020-11-16 end*/  
    FTS_FUNC_EXIT();
    return 0;
}
