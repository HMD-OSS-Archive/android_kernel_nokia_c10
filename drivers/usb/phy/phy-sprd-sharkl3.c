...
/*test 61432 ODM add for usb eye-pattern modify begin QJK*/
#define OTG_CTRL0_USB20_SET1(x) (((x) & 0x3F) << 21)
/*test 61432 ODM add for usb eye-pattern modify end QJK*/
struct sprd_hsphy {
	struct device		*dev;
	struct usb_phy		phy;
	void __iomem		*base;
...
/*test 61432 ODM add for usb eye-pattern modify begin QJK*/
void cusUsb_eyePattern_set()
{
    void __iomem *reg_base = ioremap_nocache(0x403590A8,4);
    uint32_t value;
    value = readl_relaxed(reg_base);
    value &= 0xF81FFFFF;
    value |= OTG_CTRL0_USB20_SET1(0x18);
    writel_relaxed(value,reg_base);
    iounmap(reg_base);
}
/*test 61432 ODM add for usb eye-pattern modify end QJK*/
